...
export function FeePicker({
  ...
  return (
    <form action={formAction} className="grid gap-8 lg:grid-cols-[1fr_340px] lg:items-start">
      ...
      <div>
        <div className="eyebrow">Step 1 — pick your case types</div>
        <h2 className="mt-2 text-[26px]">Choose the referrals you want</h2>
        <p className="muted mt-2 text-[14.5px]">Each figure is the base referral fee per matched case in that category. You only ever pay for referrals you accept.</p>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          background: on ? "var(--brand-100, #e6f0ee)" : "#fff",
                        }}
                      >
      ...
      <div className="lg:sticky lg:top-[88px]">
        <div className="card" style={{ background: "var(--ink)", color: "#fff", borderColor: "transparent" }}>
          ...
          <div className="mt-2 flex items-end justify-between" title="Higher-intent leads that already went through arbitration">
          ...
          <p className="mt-3 text-[12px]" style={{ color: "rgba(255,255,255,.6)" }}>
            Base = a direct referral from the finder. Post-arbitration referrals are pre-qualified leads that completed the dispute funnel.
          </p>
        </div>
        ...
        <div className="card mt-4">
          ...
          <label className="mb-2 flex items-start gap-2.5 text-[13.5px]">
            <input type="checkbox" name="notifyEmail" defaultChecked className="mt-1" style={{ width: "auto" }} />
            <span>Email me when a matching referral is available.</span>
          </label>
          <label className="mb-2 flex items-start gap-2.5 text-[13.5px]">
            <input type="checkbox" name="postArbOptIn" defaultChecked className="mt-1" style={{ width: "auto" }} />
            <span>Include higher-intent post-arbitration referrals.</span>
          </label>
          ...
          <button className="btn btn-brand btn-block btn-lg" disabled={pending || sel.length === 0}>
            {pending ? "Creating…" : sel.length === 0 ? "Pick a case type to continue" : `Create my account (${sel.length} categories)`}
          </button>
          <p className="muted mt-3 text-center text-[12.5px]">Already a member? <a href="/auth/login" className="underline" style={{ color: "var(--brand)" }}>Log in</a></p>
        </div>
      </div>
    </form>
  );
